    ...
    def print_work_hours(self, day, total_hours, half_day):
        data = [
            [
                ...
                self.convert_hours(round(abs(total_hours), 2)),
                "",
            ]
        ]
        if total_hours > self.regular_hours and not half_day:
            ...
            data[0][
                3
            ] = f"Overtime with {self.convert_hours(round(overtime_hours, 2))} hh:mm."
        if not data[0][3]:
            data[0][3] = "None"
        print(
            "\n"
            + tabulate(
                data,
                headers=["Date", "Day", "Work Hours", "Remarks"],
                tablefmt="fancy_grid",
            )
            + "\n"
        )
